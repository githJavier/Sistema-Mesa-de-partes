(function() {
    ...
    function actualizarContadorAsunto() {
        if (inputAsuntoTramite && contadorAsuntoTramite) {
    ...
    function actualizarContadorObservacion() {
        if (inputObservacion && contadorObservacion) {
    ...
    if (foliosInput) {
        foliosInput.addEventListener('input', function (e) {
    ...
    function enviarFormTramiteUsuario() {
        ...
        const btnEnviar = document.getElementById("btnEnviarTramiteUsuario");
        if (btnEnviar) {
            btnEnviar.disabled = true; // Desactiva botón
        }
    ...
    function seleccionarRemitente(valor) {
        ...
        const remitenteInput = document.getElementById('remitenteSeleccionado');
        
        if (btn && remitenteInput) {
            btn.textContent = valor.length > 50 ? valor.substring(0, 50) + '...' : valor;
            btn.setAttribute('title', valor);
            remitenteInput.value = valor;
        }
    }

    // Hacer las funciones disponibles globalmente
    window.validarTramite = validarTramite;
    window.enviarFormTramiteUsuario = enviarFormTramiteUsuario;
    window.seleccionarRemitente = seleccionarRemitente;
    window.validarCampoRequerido = validarCampoRequerido;
    window.validarFormatoExpediente = validarFormatoExpediente;
    window.validarArchivo = validarArchivo;
    window.validarFolios = validarFolios;
    window.actualizarContadorAsunto = actualizarContadorAsunto;
    window.actualizarContadorObservacion = actualizarContadorObservacion;
})();