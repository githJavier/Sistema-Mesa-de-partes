    ...
    function updateExpedientesCount() {
        ...
        if (countSpan && alertBox) {
            ...
            if (totalExpedientes === 0) {
                ...
                alertBox.querySelector("strong").innerText = "No hay expedientes por resolver";
            } else {
                countSpan.innerText = totalExpedientes;
                alertBox.querySelector("strong").innerText = `Usted tiene ${totalExpedientes} expediente(s) por resolver`;
            }
        }
    }
...
function verDetalles(codigo, tipoDocumento, asunto, fechaRegistro, remitente, flujosJSON, archivoAdjuntoJSON) {
    ...
    const supabaseBaseUrl = window.ENV.SUPABASE_URL; // URL del proyecto Supabase
...
function validarFiltroTramitesRecibidos() {
...
document.getElementById('filter-btn').addEventListener('click', function () {
    if (!validarFiltroTramitesRecibidos()) return;
