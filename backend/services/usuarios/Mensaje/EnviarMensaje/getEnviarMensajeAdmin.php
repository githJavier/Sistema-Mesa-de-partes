...
class GetEnviarMensajeAdmin {
    ...
    public function validarAdmin($datos) {
        if (!is_array($datos)) {
        ...
        if (!isset($datos['area'])) {
            $this->message = "No se ha especificado el área del usuario.";
            return false;
        }

        if (trim($datos['area']) !== 'JEFE DE SISTEMAS') {
