...
function obtenerUltimaFechaHoraActividad() {
    ...
    const fecha = card.getAttribute("data-fecha-remitente") || card.getAttribute("data-fecha");
    const hora = card.getAttribute("data-hora-remitente") || card.getAttribute("data-hora");
    return `${fecha} ${hora}`;
}
...
function insertarNuevoMensaje(data) {
    ...
    data.reverse().forEach(fila => {
        ...
        const fechaRef = fila.fecha_ultimo_mensaje_remitente || fila.fecha;
        const horaRef = fila.hora_ultimo_mensaje_remitente || fila.hora;
        ...
        const html = `
                    <div class="card mb-3 shadow-sm"
                        ...
                        data-hora="${fila.hora}"
                        data-fecha-remitente="${fila.fecha_ultimo_mensaje_remitente || ''}"
                        data-hora-remitente="${fila.hora_ultimo_mensaje_remitente || ''}">
                        
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ${fila.estado === 'En proceso'
                                ? `<button 
                                        class="badge bg-${fila.estado.toLowerCase().replace(/ /g, '-') } border-0"
                                        style="cursor: pointer;"
                                        onclick="mostrarModalResolver('${fila.id_ayuda}')">
                                        ${fila.estado}
                                </button>`
                                : `<span class="badge bg-${fila.estado.toLowerCase().replace(/ /g, '-')}">${fila.estado}</span>`
                            }
                        ...
                        <div class="card-body">
                            ...
                            <div class="text-end mt-2 d-flex justify-content-end align-items-center gap-3 flex-wrap">
                                <small class="text-muted d-flex align-items-center gap-2">
                                    ...
                                    <strong>
                                        ${fila.fecha_ultimo_mensaje_remitente && fila.hora_ultimo_mensaje_remitente
                                            ? `Último mensaje del remitente: ${fila.fecha_ultimo_mensaje_remitente} ${fila.hora_ultimo_mensaje_remitente.slice(0,5)}`
                                            : `Iniciado el ${fila.fecha} a las ${fila.hora.slice(0,5)}`
                                        }
                                    </strong>
                                </small>
                                <button class="btn btn-outline-dark btn-sm" onclick="cargarFormularioResponderMensajeAdmin('${fila.id_ayuda}')">
...
if (typeof window.intervaloMensajes === "undefined") {
    ...
    window.intervaloMensajes = setInterval(() => {
        ...
        fetch(`../../services/usuarios/Mensaje/verificarNuevosMensajesAdmin.php?ultima_actividad=${encodeURIComponent(ultimaFechaHoraActividad)}`)
            .then(res => res.json())
            .then(data => {
